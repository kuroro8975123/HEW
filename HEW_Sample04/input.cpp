...
void GamePad_Update(void)
{
	...
	for (i = 0; i<g_padCount; i++)
	{
		...
		//* Ｘボタン
		if (dijs.rgbButtons[2] & 0x80)	g_padState[i] |= BUTTON_X;
		//* Ｙボタン
		if (dijs.rgbButtons[3] & 0x80)	g_padState[i] |= BUTTON_Y;
		//* LBボタン
		...
		//* LSTICKボタン
		if (dijs.rgbButtons[8] & 0x80)	g_padState[i] |= BUTTON_LSTICK;
		//* RSTICKボタン
		if (dijs.rgbButtons[9] & 0x80)	g_padState[i] |= BUTTON_RSTICK;
